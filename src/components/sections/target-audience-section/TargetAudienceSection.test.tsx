...
describe('TargetAudienceSection', () => {
  ...
  // Removed image-related tests as the image is no longer present in the new design.
});
...
describe('Layout and styling', () => {
  it('has section element with correct minimal classes', () => {
    render(<TargetAudienceSection />);
    const section = document.querySelector('section');
    expect(section).toHaveClass('w-full', 'mb-12', 'rounded', 'px-1', 'shadow-secondary');
  });
...
describe('Content structure', () => {
  ...
  it('description has correct text alignment and margin', () => {
    render(<TargetAudienceSection />);
    const description = screen.getByText(/Wir bieten individuelle Unterstützung/i);
    expect(description.parentElement).toHaveClass('text-center', 'mb-10');
  });
